...
for t in range(nsteps-frame,nsteps):
    ...
    for k in range(7):
        print(Nsub[k],end='  ',flush=True)
    print(Nsub[-1],flush=True)

print('volume')
print(Ly**3/8)
print('box dimension')
print(lengths)
#binder_all=1/kurtosis(np.asarray(rhos).flatten(), axis=0, fisher=False, bias=False,nan_policy='omit')
#print(binder_all)
#binder_group=[]
#for j in range(400):
#    binder_group.append(1/kurtosis(np.asarray(rhos).flatten()[j::400], axis=0, fisher=False, bias=False,nan_policy='omit'))
#binder_all_err=np.sqrt(np.var(binder_group))

#print(len(rhos[0]),flush=True)
#print('binder_all='+str(binder_all)+', err='+str(binder_all_err),flush=True)
